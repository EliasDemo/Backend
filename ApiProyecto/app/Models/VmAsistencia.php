...
class VmAsistencia extends Model
{
    ...
    {
        return $this->belongsTo(VmParticipacion::class, 'participacion_id');
    }

    public function qrToken()
    {
        return $this->belongsTo(VmQrToken::class, 'qr_token_id');
    }

    /* =====================
     | Scopes útiles
     |=====================*/
    public function scopeDeSesion($q, int $sesionId)
    {
        return $q->where('sesion_id', $sesionId);
    }
    ...
    public function scopeValidadas($q)
    {
        return $q->where('estado', 'VALIDADO');
    }

    public function scopeEntreFechas($q, $desde, $hasta)
    {
        return $q->whereBetween('check_in_at', [$desde, $hasta]);
    }
}
